...
pub struct Config {
    pub delete_desktop_shortcut_files: bool,
    pub package_ids_not_to_update: Vec<String>,
}
...
impl Config {
    pub(crate) fn new() -> Result<Config, CustomError> {
        let file_content = match read_to_string(CONFIG_FILE_NAME) {
            Ok(content) => content,
            Err(_) => {
                return Err(CustomError::InvalidFileError(format!(
        ...
        match toml::from_str(&file_content) {
            Ok(config) => Ok(config),
            Err(_) => Err(CustomError::InvalidConfigError(format!(
